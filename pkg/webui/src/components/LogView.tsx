...
import { LogSliceEvent, LogSliceType } from "../api/piro_pb";
...
class LogViewImpl extends React.Component<LogViewProps, LogViewState> {
    ...
    renderSliced() {
        ...
        return <React.Fragment>
            ...
            { chunks.map((kv, i) => {
                ...
                if (isContent(chunk) && !chunk.name.startsWith("piro:")) { return (
    ...
    protected getRawLogs() {
        ...
        if (!this.state.showKubeUpdates) {
            rawLog = this.props.logs.filter(c => 
                !c.getPayload().trim().startsWith("[piro:kubernetes]")
                && !c.getPayload().trim().startsWith("[piro:status]")
            ).map(c => c.getPayload());
        }
        return rawLog.join("");
    }
    ...
    protected downloadRawLogs() {
        ...
        element.setAttribute('download', (this.props.name || "piro-logs") + ".txt");
