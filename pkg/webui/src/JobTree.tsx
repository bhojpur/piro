...
import { PiroServiceClient, ResponseStream } from './api/piro_pb_service';
import { JobStatus, ListJobsResponse, ListJobsRequest, JobPhase, SubscribeRequest, SubscribeResponse } from './api/piro_pb';
import { Header, headerStyles } from './components/header';
import { createStyles, Theme, Button, Table, TableHead, TableRow, TableCell, TableBody, Link, Grid, Tabs, Tab } from '@material-ui/core';
...
interface JobTreeProps extends WithStyles<typeof styles> {
    client: PiroServiceClient;
}

interface JobTreeState {
    branches: Map<string, JobStatus.AsObject[]>
}

class JobTreeImpl extends React.Component<JobTreeProps, JobTreeState> {

    protected eventStream: ResponseStream<SubscribeResponse> | undefined;

    constructor(props: JobTreeProps) {
        super(props);
        this.state = {
            ...
            },
            {
                ...
                render: (row: Row) => {
                    return row.jobs.map((j, i) => {
                        ...
                        return <Link href={`/job/${j.name}`} key={i}>{icon(color)}</Link>;
                    });
                }
            },
        ]
...
export const JobTree = withStyles(styles)(JobTreeImpl);
